...
SocketException::SocketException(const std::string &message, bool inclSysMsg)
  ...
  if (inclSysMsg) {
	...
	//userMessage.append("strerror()/strerror_s() - string errors\n");
  }
}
...
bool CommunicatingSocket::setNonblocking()
	throw(SocketException){
		#ifdef WIN32
		   bool blocking = false;
		   unsigned long mode = blocking ? 0 : 1;
		   return (ioctlsocket(sockDesc, FIONBIO, &mode)==0)?true:false;
		#else
		   int flags = fcntl(sockDesc, F_GETFL, 0);
		   if (flags < 0) return false;
		   flags = (flags|O_NONBLOCK);
		   return (fcntl(sockDesc, F_SETFL, flags)==0)?true:false;
		#endif
}
